...
import { removeFromRecycleBin } from '@/store/slices/recycleBinSlice';
import { APPOINTMENT_IMAGES_BUCKET } from '@/lib/storage';
...
export default function DeleteConfirmModal() {
  ...
  const handleDelete = async () => {
    ...
    if (type === 'appointment') {
      ...
      const deleted_by_name = user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Utilisateur';
      const deleted_by_email = user?.email || '';
      const { error } = await supabase.from('appointments')
        .update({ deleted_at: new Date().toISOString(), deleted_by_name, deleted_by_email })
        .eq('id', id);
      if (!error) dispatch(removeAppointment(id));
    } else if (type === 'appointment-permanent') {
      const { data: images } = await supabase.from('appointment_images')
        .select('storage_path').eq('appointment_id', id);
      if (images && images.length > 0) {
        await supabase.storage.from(APPOINTMENT_IMAGES_BUCKET).remove(images.map(i => i.storage_path));
      }
  ...
  const isSoftDelete = type === 'appointment';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => dispatch(closeDeleteConfirm())}>
      ...
      <div className="relative bg-white rounded-2xl shadow-modal w-full max-w-sm animate-scale-in p-6" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="font-display font-bold text-ink text-center mb-2">
          {isSoftDelete ? 'Déplacer vers la corbeille' : 'Confirmer la suppression'}
        </h3>
        <p className="text-ink-muted text-sm text-center mb-6">
          {isSoftDelete
            ? 'Ce rendez-vous sera déplacé vers la corbeille. Vous pourrez le restaurer plus tard.'
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleDelete} className="btn-danger flex-1 justify-center" disabled={loading}>
            {loading ? 'Suppression...' : isSoftDelete ? 'Déplacer' : 'Supprimer'}
          </button>
        </div>
      </div>
    </div>
